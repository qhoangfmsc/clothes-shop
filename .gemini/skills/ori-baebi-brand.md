# Ori Baebi — Brand Identity & Storytelling

## Brand Essence

**Ori Baebi** is a contemporary womenswear label that lives at the intersection of **soft femininity** and **unapologetic confidence**. The name itself is a declaration — "Ori" (original, origin, one-of-a-kind) fused with "Baebi" (a term of endearment, youthful irreverence, and self-love). Every piece whispers _"I know who I am"_ while draping the wearer in textures that feel like a love letter to her own skin.

The brand does not chase trends — it **curates moods**. Each collection is a chapter of the same story: a girl who wakes up in sun-drenched linen, ties ribbons in her hair, and walks into a room knowing every eye will follow.

---

## Brand DNA

### Core Identity
- **Full Name**: Ori Baebi
- **Tagline Concepts**: _"Softly Slay"_ · _"Born Tender, Made Bold"_ · _"Where Cute Meets Couture"_
- **Brand Archetype**: The Muse — ethereal yet magnetic, delicate yet deliberate
- **Target Audience**: Women 18–30 who live between Pinterest mood boards and Paris Fashion Week front rows. She is the girl who curates her life with the same intention she curates her closet.

### Brand Personality
| Trait | Description |
|---|---|
| **Dịu dàng (Gentle)** | Every silhouette flows, never constricts. Fabrics kiss the body rather than grip it. |
| **Slay** | Behind the softness is a girl who knows her angles. Styling is intentional, never accidental. |
| **Cute** | Playful details — bows, scalloped edges, pastel polka dots — that wink at girlhood without being juvenile. |
| **Thanh lịch (Elegant)** | Restraint in proportion. No excess. The kind of elegance that comes from knowing what to leave off. |
| **Chuyên nghiệp (Professional)** | Runway-grade construction. Every seam has purpose. International fashion show standard. |

---

## The Ori Baebi Girl

She is not one archetype — she is a spectrum:

- **The Morning Version**: Hair still damp, wearing an ivory slip skirt with a lace-trimmed camisole, ballet flats, and a crossbody bag with gold hardware. She looks like she's about to step into a Parisian café.
- **The Afternoon Version**: A sage-green halter top with eyelet embroidery tucked into a gingham midi skirt. She carries a structured hobo bag in dusty blue. She is effortless but unmistakably styled.
- **The Evening Version**: A chocolate-brown bodysuit under a sheer chiffon skirt, ankle boots, and layered gold necklaces. She doesn't walk into a room — she arrives.

She never tries too hard. She never needs to.

---

## Aesthetic Pillars

### 1. Romantic Feminism
The foundation of Ori Baebi is **neo-romantic femininity** — a rejection of the idea that softness is weakness. Inspired by:
- **Runway References**: Simone Rocha's sculpted tulle, Sandy Liang's bow-forward pieces, Cecilie Bahnsen's voluminous cotton poplin
- **Visual Codes**: Ribbons as structural elements (not just decoration), lace used as overlay and texture (not lingerie), floral prints that feel handpainted rather than mass-printed
- **Key Fabrics**: Broderie anglaise, Swiss dot cotton, chantilly lace, satin-back crepe

### 2. Coquette Minimalism
Where **coquette** meets **quiet luxury** — Ori Baebi strips coquette styling back to its essentials:
- No excessive bows. One perfectly placed ribbon at a neckline says more than twelve.
- **Polka dots** are used sparingly — a midi skirt, a hair ribbon — never head-to-toe.
- **Lace trims** appear at hems and necklines as finishing details, like a whispered secret.
- **Ruffle construction**: Soft, controlled ruffles — not costume ruffles. Think Giambattista Valli's resort collections scaled to everyday wearability.

### 3. Undone Elegance
The styling philosophy of Ori Baebi is **imperfect perfection** — the "I woke up like this" illusion that actually takes a trained eye:
- Hair is always slightly undone — center-parted, softly waved, or pulled into a low bun with face-framing pieces
- Tops are slightly off-shoulder, not perfectly placed
- Skirts have asymmetric hemlines or bias cuts that move with the body
- Leg warmers, ballet flats, and Mary Janes create a balletcore foundation without costume
- **Styling rule**: One unexpected element per outfit (a structured bag with a flowy dress, a cardigan left unbuttoned over a tube top)

### 4. East-West Fusion
As a Vietnamese-born label with global ambitions, Ori Baebi subtly weaves **Asian design sensibility** into Western silhouettes:
- Clean lines reminiscent of áo dài construction — high necklines, fitted bodices that flare below the waist
- Silk fabrics that reference traditional Vietnamese textile heritage
- Delicate beadwork and embroidery techniques inspired by Southeast Asian craft
- Color palette influenced by both European fashion houses and the soft, misty tones of Vietnamese landscapes

---

## Color Universe

> All color tokens are defined in `src/styles/design-system.css`. See `DESIGN.md` for full palette documentation.

### Canvas Palette — "Champagne Boutique"
| Color | Hex | Usage |
|---|---|---|
| **Pearl Cream** | `#FBF8F1` | Primary page bg — the warm canvas of the brand |
| **Champagne Cream** | `#F5EFE0` | Secondary bg — soft champagne warmth |
| **Warm Linen** | `#EDE5D4` | Card surfaces, panels, garment backdrop |

### Accent Families — "The Product Universe"
Each product category has its own color family, creating visual richness across the store:

| Family | Colors | Inspired By |
|---|---|---|
| **Gold** | `#C9A96E` `#D4B978` `#E8D5A3` | Jewelry, gold hardware, premium CTA |
| **Dusty Rose** | `#D4A5A5` `#E8C4C0` `#F5E0DC` | Romantic tops, floral prints, ribbons |
| **Sage** | `#A3B18A` `#C5CEB5` `#E2E8D5` | Spring pieces, halter tops, nature-inspired |
| **Lavender** | `#B8A5C8` `#D4C8E0` `#EDE6F2` | Evening lace overlays, skirt fabrics |
| **Dusty Blue** | `#8FA3B4` `#B5C8D6` `#DAE4EC` | Bags, hardware, denim-adjacent pieces |
| **Chocolate** | `#5C4033` `#8B7060` `#C4AE98` | Leather goods, evening wear, structure |
| **Soft Yellow** | `#F0E4A6` `#FAF3DC` `#D4C07A` | Gingham, summer collection, warm glow |

### Contrast — "Noir Accents" (use sparingly)
| Color | Hex | Usage |
|---|---|---|
| **Noir** | `#0A0A08` | Gallery panel, hero sections, headings |
| **Obsidian** | `#1A1917` | Body text on light backgrounds |
| **Charcoal** | `#2C2926` | Secondary text, descriptions |

---

## Product Categories

### 1. TOPS — _"Second Skin"_
The first layer of the Ori Baebi wardrobe. Every top is designed to make the wearer feel like the fabric was woven specifically for her.

| Sub-Category | Design Language | Key Details |
|---|---|---|
| **Camisoles** | Spaghetti straps or adjustable ties, bias-cut | Lace trims at hem and neckline, French seam construction |
| **Halter Tops** | Tied at nape, open back | Eyelet embroidery, broderie anglaise, scalloped edges |
| **Tank Tops** | Fitted, slightly cropped | Rib knit or smooth jersey, subtle ruching at sides |
| **Off-Shoulder** | Draped neckline, romantic fit | Balloon sleeves optional, elastic shirring |
| **Cardigans** | Slightly oversized, soft knit | Mother-of-pearl buttons, pointelle detailing |
| **Corset Tops** | Structured, boned construction | Satin finish, ribbon lacing at back |

### 2. SKIRTS — _"The Way She Moves"_
Ori Baebi skirts are never static. They are designed to flow, to catch light, to make the wearer want to twirl.

| Sub-Category | Design Language | Key Details |
|---|---|---|
| **Slip Skirts** | Midi length, bias-cut satin | Side slits, concealed elastic waist, fluid drape |
| **Midi Skirts** | Below-knee, A-line or asymmetric | Gingham, polka dot, or floral print |
| **Mini Skirts** | Above-knee, flared or pleated | Ruffle hem, tiered construction |
| **Wrap Skirts** | Adjustable tie, versatile styling | Floral silk, overlap panel |
| **Lace Skirts** | Overlay construction | Lined with silk or cotton slip |

### 3. BAGS — _"Carried With Intention"_
Every Ori Baebi bag is a sculpture you carry. Not just an accessory — a statement of identity.

| Sub-Category | Design Language | Key Details |
|---|---|---|
| **Hobo Bags** | Slouchy crescent, soft leather | Oversized ring hardware, magnetic closure |
| **Shoulder Bags** | Structured, medium size | Gold/brass hardware, chain strap option |
| **Clutches** | Evening-ready, minimal | Satin or velvet, jeweled clasp |
| **Mini Bags** | Crossbody, petite | Beaded strap, playful shapes |
| **Tote Bags** | Everyday carry, canvas or leather | Embossed logo, internal organizer |

### 4. JEWELRY — _"The Finishing Whisper"_
Ori Baebi jewelry is never loud. It is the final detail that makes someone lean in closer.

| Sub-Category | Design Language | Key Details |
|---|---|---|
| **Necklaces** | Layered chains, chokers, pendants | Freshwater pearls, gold vermeil, delicate chains |
| **Earrings** | Studs, drops, hoops | Mother-of-pearl, enamel flower details |
| **Rings** | Stackable, minimal | Signet styles, twisted bands |
| **Bracelets** | Beaded, chain, or cuff | Charm details, adjustable sizing |
| **Hair Accessories** | Ribbons, clips, headbands | Satin bows, velvet scrunchies, pearl pins |

---

## Visual Language — Digital Presence

### Photography Direction
- **Lighting**: High-contrast editorial noir. Dramatic shadows, directional light. Candlelit warmth for intimate shots. Studio flash only for clean product detail shots.
- **Setting**: Dark studios, architectural noir, candlelit interiors, marble surfaces with gold accents. Never generic white-wall catalog.
- **Model Styling**: Editorial confidence. Intentional styling, sculpted hair, bold but refined makeup. The Ori Baebi girl doesn't whisper — she commands.
- **Composition**: Cinematic framing. Generous negative space (dark). The garment emerges from shadow. Close-up details on gold hardware, stitching, texture.
- **Post-Production**: Warm-toned, high contrast, rich blacks. Think Peter Lindbergh meets Bottega Veneta campaign. Champagne/amber color grading.

### Website Experience
- **First Impression**: The website should feel like entering a gilded atelier at midnight — dark, intimate, every surface gleaming with quiet luxury
- **Scroll Experience**: Slow, cinematic. Each section reveals like a scene change in a runway show
- **Typography**: Quiche Display for editorial voice (headings), Inter Tight for precision (UI). Wide tracking on uppercase labels. Mixed case for storytelling.
- **Interactions**: Gold accent reveals on hover — border glow, subtle lift, text shimmer. Never jarring, always refined.
- **Gallery**: Products presented as editorial art, not catalog. Full-bleed imagery on noir backgrounds. Context over commerce. Gold glow on featured pieces.

### Brand Voice — Copy Style
| Context | Tone | Example |
|---|---|---|
| **Product Names** | Poetic, evocative | _"Daphne Slip Skirt"_, _"Rosemary Halter"_, _"Luna Hobo Bag"_ |
| **Product Descriptions** | Sensory, tactile | _"Cut on the bias from washed silk, the Daphne falls like water against the body. Finished with French seams and a concealed elastic waist."_ |
| **Collection Titles** | Narrative, seasonal | _"Somewhere Between Morning & Noon"_, _"Letters Never Sent"_, _"The Garden After Rain"_ |
| **Social Captions** | Intimate, second-person | _"This is the skirt you'll reach for when you want to feel like the main character."_ |
| **Campaign Headlines** | Bold, minimal | _"Softly."_, _"Not Sorry."_, _"She Knew."_ |

---

## Runway & Fashion Show Direction

### Show Concept — _"The Living Lookbook"_
Ori Baebi shows are not traditional catwalks. They are **experiential installations** — part fashion show, part art exhibition, part sensory journey.

#### Stage Design
- **Set**: Minimal architectural structures — arched doorways, draped fabric panels, scattered dried flowers
- **Flooring**: Raw linen or bleached wood. Never a standard black runway.
- **Lighting**: Warm, dappled. As if sunlight is filtering through curtains. Candle-lit accents for evening collections.
- **Seating**: No standard folding chairs. Velvet cushions on bleached-wood benches. Guests feel like they're in someone's living room.

#### Walk Style
- Models walk **barefoot** or in ballet flats — never towering heels
- Pace is **slower than industry standard** — each look deserves its moment
- Models may pause, turn slowly, touch the fabric — the clothing is alive
- No stern expressions. Soft, knowing smiles. The Ori Baebi girl is in on the secret.

#### Music & Soundscape
- Ambient, ethereal. Think Ryuichi Sakamoto, Nils Frahm, or Debussy's _Clair de Lune_ reimagined
- Layered with field recordings — birdsong, rain on glass, fabric rustling
- Builds subtly — the final look walks in near-silence, then a swell of strings

#### Show Finale
- All models return, walking in pairs, holding hands or linking arms
- Final tableau: models stand still in formation, lights dim to a single warm spotlight
- A projection of the collection title appears on the back wall in the brand typeface

---

## Seasonal Collection Framework

### Spring/Summer — _"Mornings"_
- Focus: Lightweight fabrics, open silhouettes, garden-party palette
- Hero Pieces: Eyelet halter tops, floral slip skirts, woven straw bags
- Colors: Sage, cream, soft yellow, dusty rose
- Campaign Mood: Barefoot in a meadow, golden hour, wildflowers

### Autumn/Winter — _"After Hours"_
- Focus: Layered textures, richer tones, structured accessories
- Hero Pieces: Knit cardigans, velvet midi skirts, leather hobo bags, satin corset tops
- Colors: Chocolate, deep plum, warm taupe, ivory
- Campaign Mood: Candlelit apartments, vintage furniture, fog on windows

### Resort — _"Between Places"_
- Focus: Travel-ready versatility, effortless mix-and-match
- Hero Pieces: Wrap skirts, linen camisoles, beaded crossbody bags
- Colors: Ocean-washed blues, sand, pale lavender
- Campaign Mood: Mediterranean balconies, old-world hotels, morning coffee

---

## Brand Inspirations & References

### Fashion Houses
| Brand | What We Take From Them |
|---|---|
| **Sandy Liang** | Bow construction, playful femininity rooted in NYC cool |
| **Simone Rocha** | Sculptural romance, volume as emotion |
| **Cecilie Bahnsen** | Cotton as couture, white as a statement |
| **Jacquemus** | Mediterranean warmth, body-positive minimalism |
| **The Row** | Quiet luxury, fabric-first philosophy |
| **Miu Miu** | Intellectual girlhood, subversive cute |
| **Paloma Wool** | Art-world adjacency, soft surrealism |

### Visual References
- **Film**: Sofia Coppola's _Marie Antoinette_, Wong Kar-wai's _In the Mood for Love_
- **Photography**: Petra Collins, Tim Walker, Harley Weir
- **Art**: Berthe Morisot, Mary Cassatt, contemporary Vietnamese silk painting
- **Architecture**: Tadao Ando's concrete minimalism meets Vietnamese colonial villas

---

## Competitive Positioning

```
                    ┌─────────────────────────────┐
                    │      HIGH FASHION            │
                    │                              │
         Simone    │     ★ ORI BAEBI              │  The Row
         Rocha     │     (sweet spot)              │
                    │                              │
    ◄───────────────┼──────────────────────────────┼──────────────►
    PLAYFUL /       │                              │  MINIMAL /
    CUTE            │                              │  RESTRAINED
                    │                              │
         Sandy     │                              │  COS
         Liang     │                              │
                    │      ACCESSIBLE              │
                    └─────────────────────────────┘
```

Ori Baebi sits in the **upper-left quadrant** — unmistakably high-fashion in quality and presentation, but with a warmth, playfulness, and emotional accessibility that ultra-minimalist brands lack.

---

## Key Rules for All Brand Touchpoints

1. **Never cold, always warm** — even the darkest palette should feel inviting
2. **Softness is strength** — every design decision should reinforce this philosophy
3. **Show, don't tell** — let the fabric, the cut, the drape speak. Copy supports, never overshadows.
4. **One surprise per piece** — a hidden ribbon, an unexpected slit, a pearl button where you'd expect plastic
5. **International standard, local soul** — construction and presentation rival Parisian houses, but the heart is Vietnamese
6. **The girl comes first** — the brand serves the wearer, not the other way around. Every piece should make her feel like the most beautiful version of herself.
